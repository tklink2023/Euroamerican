...
If lSession 
	...
	cQuery += "AND CR_TIPO = 'PC' " + CRLF     //-- Tipo do Documento   {PC – Pedido de Compras ; AE – Autorização de Entrega ; **CC – Cartão Corporativo ; **A1 – Cad. Clientes ; **A2 – Cad. Fornecedores ; **A3 – Cad. Vendedores ; **A4 – Cad. Transportadora ; **B1 – Cad. Produtos ; **ED – Cad. Naturezas } "
	...
	cHtml += '	<h2><i class="fa fa-calendar-check-o fa-1x"></i> Liberação Pedidos de Compras</h2>' //<i class="fa fa-tasks fa-1x"></i>
	cHtml += '	<hr/>'
	cHtml += '	<p></p>
	...
	Do While !(cAlias)->( Eof() )
		...
		cHtml += '    						<li><a href="u_bep0201A.apw?filexc='+(cAlias)->C7_FILIAL+'&numped='+(cAlias)->C7_NUM+'&grpapr='+(cAlias)->AL_COD+'&codaprov='+(cAlias)->CR_USER+'&nivel='+(cAlias)->CR_NIVEL+'&nopc=1">Visualizar</a></li>
		cHtml += '    						<li><a href="u_bep0201A.apw?filexc='+(cAlias)->C7_FILIAL+'&numped='+(cAlias)->C7_NUM+'&grpapr='+(cAlias)->AL_COD+'&codaprov='+(cAlias)->CR_USER+'&nivel='+(cAlias)->CR_NIVEL+'&nopc=2">Liberar</a></li>
		If lSuperior
			cHtml += '    						<li><a href="u_bep0201A.apw?filexc='+(cAlias)->C7_FILIAL+'&numped='+(cAlias)->C7_NUM+'&grpapr='+(cAlias)->AL_COD+'&codaprov='+(cAlias)->CR_USER+'&nivel='+(cAlias)->CR_NIVEL+'&superior='+SAK->AK_APROSUP+'&nopc=3">Transferir Superior</a></li>
		EndIf
		cHtml += '    						<li><a href="u_bep0205A.apw?filexc='+(cAlias)->C7_FILIAL+'&codfor='+(cAlias)->C7_FORNECE+'&lojfor='+(cAlias)->C7_LOJA+'&nopc=1">Consultar Fornecedor</a></li>
		cHtml += '  					</ul>
		cHtml += '					</div>
		cHtml += '				</td>
