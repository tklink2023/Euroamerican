...
If lSession
	...
	cQuery := "SELECT COUNT(*) AS QUANTIDADE " + CRLF
	...
	cQuery += "AND CR_NIVEL = '" + HttpGet->nivel + "' " + CRLF
	cQuery += "AND CR_STATUS = '03' " + CRLF
	cQuery += "AND SCR.D_E_L_E_T_ = '*' " + CRLF

	TCQuery cQuery New Alias "TMPVEZ"
	dbSelectArea("TMPVEZ")
	dbGoTop()

	If !TMPVEZ->( Eof() )
		If TMPVEZ->QUANTIDADE > 0
			nVezes := TMPVEZ->QUANTIDADE
		EndIf
	EndIf
	...
	If nVezes > 0
		cHtml += '<?php if($acao=="sucesso"){ ?>'+CRLF
		cHtml += '    <script>'+CRLF
		cHtml += '        $(document).ready(function(){'+CRLF
		cHtml += '            $("#modalAviso").modal();'+CRLF
		cHtml += '        });'+CRLF
		cHtml += '    </script>'+CRLF
		cHtml += '<? } ?>'+CRLF

		cHtml += '<div class="modal fade" id="modalAviso" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">'+CRLF
		cHtml += '  <div class="modal-dialog" role="document">'+CRLF
		cHtml += '    <div class="modal-content">'+CRLF
		cHtml += '      <div class="modal-header">'+CRLF
		cHtml += '        <h3 class="modal-title" id="exampleModalLabel">Pedido Aprovado Anteriormente</h3>'+CRLF
		cHtml += '        <button type="button" class="close" data-dismiss="modal" aria-label="Fechar">'+CRLF
		cHtml += '          <span aria-hidden="true">&times;</span>'+CRLF
		cHtml += '        </button>'+CRLF
		cHtml += '      </div>'+CRLF
		cHtml += '      <div class="modal-body">Este pedido de compras já foi aprovado por ' + AllTrim( Str( nVezes ) ) + ' vez(es) antes de ser bloqueado novamente, portanto, houve alterações após sua aprovação!</div>'+CRLF
		cHtml += '      <div class="modal-footer">'+CRLF
		cHtml += '        <button type="button" class="btn btn-secondary" data-dismiss="modal">Fechar</button>'+CRLF
		cHtml += '      </div>'+CRLF
		cHtml += '    </div>'+CRLF
		cHtml += '  </div>'+CRLF
		cHtml += '</div>'+CRLF
	...
	If HttpGet->nopc == "3"
		cHtml += '  <h2>Transferência Superior - Pedido de Compra</h2>'+CRLF
		cHtml += '  <hr/>'+CRLF
		cHtml += '  <form method="POST" id="formpc" action="u_bep0201C.apw" class="col-md-12" style="margin-bottom: 10px;">'+CRLF
	Else
		cHtml += '  <h2>Liberação Pedido de Compra</h2>'+CRLF
		cHtml += '  <hr/>'+CRLF
	...
	*/
	...
	cHtml += '        			<th>Obs. SC</th>'+CRLF
	cHtml += '      		</tr>'+CRLF
	cHtml += '    		</thead>'+CRLF
	cHtml += '			<tbody>'+CRLF
	
	Do While SC7->(!Eof()) .And. SC7->C7_FILIAL == Alltrim(HttpGet->filexc) .And. SC7->C7_NUM == Alltrim(HttpGet->numped)
		cQuery := "SELECT ISNULL( C1_OBS, '') AS OBSERVACAO " + CRLF
		cQuery += "FROM " + RetSqlName("SC7") + " AS SC7 WITH (NOLOCK) " + CRLF
		cQuery += "INNER JOIN " + RetSqlName("SC1") + " AS SC1 WITH (NOLOCK) ON C1_FILIAL = C7_FILIAL " + CRLF
		...
		cQuery += "AND C7_ITEM = '" + SC7->C7_ITEM + "' " + CRLF
		cQuery += "AND SC7.D_E_L_E_T_ = ' ' " + CRLF

		TCQuery cQuery New Alias "SOLOBS"
		dbSelectArea("SOLOBS")
		dbGoTop()
		...
		Do While !SOLOBS->( Eof() )
			cSolObs += AllTrim( SOLOBS->OBSERVACAO ) + " "
		...
		cHtml += '    							<li><a href="u_bep0206B.apw?codpro='+SC7->C7_PRODUTO+'&nopc=2">Consultar Produto</a></li>
		cHtml += '  						</ul>
		cHtml += '						</div>
		cHtml += '					</td>
		...
		cHtml += '        			<td nowrap>'+Alltrim(SC7->C7_OBS)+'</td>'+CRLF
		...
		cHtml += '        			<td nowrap>'+AllTrim(cSolObs)+'</td>'+CRLF
	...
	If HttpGet->nopc == "2"
		...
		cHtml += '    	<a href="U_BeP0201.apw" class="btn btn-default">Cancelar</a>'+CRLF
		cHtml += '	  </div>'+CRLF
