...
Local cUsrIncRNC		:= Alltrim(SuperGetMV("ES_URNCINC",.T.,"")) // Usuarios com acesso a Alteração RNC
...
IF nOpc = 3
   IF ( Upper(Alltrim(cUserName)) == Upper(Alltrim(cUsrIncRNC))  )
		 ...
		 M->UI_TERMINO := DDATABASE
		 M->UI_XUSER   := CUSERNAME
