...
#INCLUDE "TOPCONN.CH" 
#INCLUDE "RWMAKE.CH"             // Customizado Euroamerican
#DEFINE ENTER chr(13) + chr(10)	 // Customizado Euroamerican
...
#DEFINE MAXITEMC   045 //30//043//035	// Máxima de caracteres por linha de produtos/serviços (Personalização Euroamerican)
#DEFINE MAXMENLIN  080					// Máximo de caracteres por linha de dados adicionais
#DEFINE MAXMSG     011 //013			// Máximo de dados adicionais por página  (Personalização Euroamerican)
#DEFINE MAXVALORC  012 //009			// Máximo de caracteres por linha de valores numéricos  (Personalização Euroamerican)
#DEFINE MAXCODPRD  060 //055 //050		// Máximo de caracteres do codigo de produtos/servicos conforme o tamanho do quadro "Cod. prod"  (Personalização Euroamerican)
// - VERIFICAR SEMPRE QUANDO ATUALIZAR - 2022 - CUSTOM - Personalização Euro
/*/
...
If nTipo <> 1
	...
	PRIVATE oFont15N   := TFontEx():New(oDanfe,"Times New Roman",14,14,.T.,.T.,.F.)// Personalizacao Euro
...
Local aAux          := {}
Local aUF           := {}
//Local aMensagem     := {} // Customizado Euroamerican
Local aEspVol       := {}
Local aResFisco     := {}
Local aEspecie      := {}
Local aIndImp	    := {}
...
//---------------------------------------------------------
//Customizado Euroamerican
//---------------------------------------------------------
Local cMen2 := cMen3 := cMen4 := cMen5 := ""  
Local aMsgONU := {}
...
Private aMensagem  := {}
Private cEntr      := ""   //Endereco de Entrega do Cliente/Fornecedor  
Private cRetira    := ""
...
If Type("oIdent:_tpAmb:TEXT")<>"U" .And. oIdent:_tpAmb:TEXT=="2"
	cAux := "DANFE emitida no ambiente de homologação - SEM VALOR FISCAL"
	While !Empty(cAux)
		aadd(aMensagem,{ SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN) - 1, MAXMENLIN)), .T. })
		cAux := SubStr(cAux,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN), MAXMENLIN) + 1)
	EndDo
EndIf
...
fMensagens(StrZero(Val(oIdent:_NNf:Text),9), oIdent:_Serie:Text) //Mensagens Complementares Customizadas

aAdd(aMensagem, {"Caso não receba boletos de cobrança até 3 dias antes do vencto, comunicar Dep Cobrança.", .T.})
			
If oDet[nX]:_Prod:_cProd:TEXT == "ME.0044"
	aAdd(aMensagem, {chr(13) + chr(10), .T. })
	aAdd(aMensagem, {"Estabelecemos o prazo de 60 dias a partir da emissão desta nota fiscal para o retorno dos containers.", .T. })
	aAdd(aMensagem, {"O não cumprimento do prazo e/ou retorno em condições inadequadas poderão ocasionar cobrança financeira.", .T. })
	lImpCont := .F.
EndIf    

//----------------------------------------------
//Customizado Euroamerican - FIM
//----------------------------------------------
...
// Identifica e Destaca mensagens especificas no Array
nLenMensagens:= Len(aMensagem)
For nX := 1 To Min(nLenMensagens, MAXMSG)
		If "|"  $ aMensagem[nX][1]  
			nPos1:=AT( "|",aMensagem[nX][1])     // Caracater da mensgam de pedido de venda - Inicio
			nPos2:=RAT("|",aMensagem[nX][1])     // Caracater da mensgam de pedido de venda - Final

			cMsgPedVen:=Substr(aMensagem[nX][1], nPos1+1, nPos2)
			cMsgPedVen:=Strtran(cMsgPedVen, "|,", ", ")
			cMsgPedVen:=Strtran(cMsgPedVen, "|", "")

			// Retira o Pedido de Venda da Mensagem
			cRestMsg := Substr(aMensagem[nx][1], 1, nPos1-1)

			IF Len(Alltrim(cRestMsg)) > 0
				aMensagem[nX][1]:= cRestMsg 
				
				// Cria uma linha especifica para pedido
			    aadd(aMensagem, { cMsgPedVen , .F. } )   
			else
				aMensagem[nX][1]:= cMsgPedVen 	
			Endif 
		End
Next nX

// Inclui Codigo ONU no final da mensagem
For nX := 1 To Len(aMsgONU)
	aadd(aMensagem, { aMsgONU[nX][1] , .F. } )  
Next nX
...
For nX := 1 To Min(nLenMensagens, MAXMSG)
	If "Pedido de Venda: " $ aMensagem[nX][1]  
		nLin:=nlin+2 //3
		oDanfe:Say(nLin,002,aMensagem[nX][1],oFont15N:oFont) 
	Else
		oDanfe:Say(nLin,002,aMensagem[nX][1],oFont07:oFont)  
	End
	nLin:= nLin+10
Next nX
//--------------------------------------------------
...
//--------------------------------------------------
Static Function fMensagens(cNFis, cSeri)

Local cCobC  := ""   //Endereco de Cobranca do Cliente/Fornecedor
Local cPedC  := ""   //Pedido do Cliente
Local cPedF  := ""   //Pedido do Vendas
Local cLotC  := ""   //Lote de Amostra
Local cMenN  := ""   //Mensagem para Nota
Local cMenP  := ""   //Mensagem Padrao para o Produto   
Local cMsgVCO:= ""   //Mensagem Venda Conta e Ordem	
Local nTam :=133//65
Local i

If mv_par04 == 2  //Dados para NF Saida
    IF Left(cFilAnt,2) = "01" .and. Left(cFilAnt,2) = "06"
    	cSeri := StrZero(val(cSeri),2)
    endif	

	//Grava informacoes do SF2
	dbSelectArea("SF2")
	dbSetOrder(1)
	dbSeek(xFilial("SF2")+ cNFis + cSeri) 
	
	
	cEspc := Alltrim(SF2->F2_ESPECI1) + ;
	Iif(Len(Alltrim(SF2->F2_ESPECI2))<>0,"/" + Alltrim(SF2->F2_ESPECI2),"") + ;
	Iif(Len(Alltrim(SF2->F2_ESPECI3))<>0,"/" + Alltrim(SF2->F2_ESPECI3),"") + ;
	Iif(Len(Alltrim(SF2->F2_ESPECI4))<>0,"/" + Alltrim(SF2->F2_ESPECI4),"") 
  				
	cQtde := Alltrim(Str(SF2->F2_VOLUME1)) + ;
	Iif(SF2->F2_VOLUME2<>0,"-" + Alltrim(Str(SF2->F2_VOLUME2)),"") + ;
	Iif(SF2->F2_VOLUME3<>0,"-" + Alltrim(Str(SF2->F2_VOLUME3)),"") + ;
	Iif(SF2->F2_VOLUME4<>0,"-" + Alltrim(Str(SF2->F2_VOLUME4)),"") + ;
	Iif(SF2->F2_VOLUME5<>0,"-" + Alltrim(Str(SF2->F2_VOLUME5)),"") + ;
	Iif(SF2->F2_VOLUME6<>0,"-" + Alltrim(Str(SF2->F2_VOLUME6)),"") + ;
	Iif(SF2->F2_VOLUME7<>0,"-" + Alltrim(Str(SF2->F2_VOLUME7)),"")    
	
		//Grava informacoes do SC5
	dbSelectArea("SC5")
	dbSetOrder(8)
	dbSeek(xFilial("SC5") +  cNFis + cSeri)
                     
    //Cadastro de Clientes
	dbSelectArea("SA1")
	dbSetOrder(1)
	dbSeek(Iif(Subs(SC5->C5_CLIENTE, 1, 1) == "R", "01", "00") +  SC5->C5_CLIENTE + SC5->C5_LOJACLI)
	
	cMenN += AllTrim(SC5->C5_MENNOTA)   
	
	//Venda Conta e Ordem
	cQry := " SELECT " 
	cQry += " 		CASE WHEN (SELECT COUNT(1) FROM "+RetSqlName("SD2")+" WHERE D_E_L_E_T_ = '' AND D2_FILIAL = F2_FILIAL AND D2_SERIE = F2_SERIE AND D2_DOC = F2_DOC AND D2_TES IN ('711', '521')) > 0 THEN " 
	cQry += " 	 		 'REM.POR CTA.ORDEM REF.A NF DE VENDA DE N. ' + F2_SERIE + F2_DOC + ' DE ' + CONVERT(VARCHAR, CONVERT(DATETIME, F2_EMISSAO), 103) "
	cQry += " 			 WHEN (SELECT COUNT(1) FROM "+RetSqlName("SD2")+" WHERE D_E_L_E_T_ = '' AND D2_FILIAL = F2_FILIAL AND D2_SERIE = F2_SERIE AND D2_DOC = F2_DOC AND D2_TES IN ('722', '517')) > 0 THEN "
	cQry += " 	 		 'MERCADORIA REMETIDA ATRAVES DA NF DE REMESSA N. ' + F2_SERIE + F2_DOC + ' DE ' + CONVERT(VARCHAR, CONVERT(DATETIME, F2_EMISSAO), 103) " 
	cQry += " 	 		 ELSE '' END MSG "
	cQry += " FROM "
	cQry += " 		" + RetSqlName("SF2") + "  "
	cQry += " WHERE "						
	cQry += " 		D_E_L_E_T_ = '' "
	cQry += " 		AND F2_FILIAL + F2_SERIE + F2_DOC =	(SELECT	"
	cQry += " 													CASE WHEN SC51.C5_PEDVINC != '' THEN (SELECT SC52.C5_FILIAL + SC52.C5_SERIE + SC52.C5_NOTA FROM " + RetSqlName("SC5") + " SC52 WHERE SC52.D_E_L_E_T_ = '' AND SC51.D_E_L_E_T_ = '' AND SC51.C5_FILIAL  = SC52.C5_FILIAL AND SC51.C5_PEDVINC = SC52.C5_NUM) ELSE (SELECT SC52.C5_FILIAL + SC52.C5_SERIE + SC52.C5_NOTA FROM " + RetSqlName("SC5") + " SC52 WHERE SC52.D_E_L_E_T_ = '' AND SC51.D_E_L_E_T_ = '' AND SC51.C5_FILIAL  = SC52.C5_FILIAL AND SC52.C5_PEDVINC = SC51.C5_NUM) END PEDVINC "
	cQry += " 											FROM "
	cQry += " 													" + RetSqlName("SC5") + " SC51 "
	cQry += " 											WHERE "
	cQry += " 													SC51.D_E_L_E_T_  = '' "
	cQry += " 													AND SC51.C5_FILIAL  = '" + SC5->C5_FILIAL + "' "
	cQry += " 													AND SC51.C5_NUM = '" + SC5->C5_NUM + "') "      
	
	
	TCQUERY cQry NEW ALIAS QRYVCO
	If !QRYVCO->(EOF())      
		cMsgVCO := QRYVCO->MSG
	EndIf
	QRYVCO->(dbCloseArea())
	...
	/*While !EOF("SC6") .And. SC6->C6_NUM == SC5->C5_NUM
		...
		If lPrm1 .And. (Len(AllTrim(SC6->C6_PEDCLI)) > 0 .Or. Len(AllTrim(SC6->C6_LOTECTL)) > 0)    
		    IF ALLTRIM(cPedC) <> AllTrim(SC6->C6_PEDCLI)
					cPedC := AllTrim(SC6->C6_PEDCLI) 
			ENDIF		
			cLotC := SC6->C6_LOTECTL
			lPrm1 := .F.
		ElseIf !lPrm1 .And. (Len(Alltrim(SC6->C6_PEDCLI)) > 0 .Or. Len(AllTrim(SC6->C6_LOTECTL)) > 0)   
			IF ALLTRIM(cPedC) <> AllTrim(SC6->C6_PEDCLI)
				cPedC += "-" + AllTrim(SC6->C6_PEDCLI) 
	   		Endif	
			cLotC := "-" + SC6->C6_LOTECTL  
	...
	//Cadastro de Clientes  - Adicionado 20/12/17
	dbSelectArea("SA1")
	dbSetOrder(1)
	MsSeek(xFilial("SA1")+SF2->F2_CLIENTE+SF2->F2_LOJA)
	
	
	//Grava informacoes do SA1 / SA2
	If !(SC5->C5_TIPO $ "B/D")   //Normal

		cCobC += AllTrim(SA1->A1_ENDCOB) + Iif(!Empty(SA1->A1_MUNC),"/" + AllTrim(SA1->A1_MUNC), "") + Iif(!Empty(SA1->A1_ESTC),"/" +  AllTrim(SA1->A1_ESTC), "")

		//Acrescenta Mensagem Padrao 
		cMenN += Alltrim(Iif(SA1->A1_LAUDO == "S", " * ACOMPANHA LAUDO * ", "")) + " / OBS: " + AllTrim(SA1->A1_U_ENTRG) + " / RECEB.: " + SA1->A1_HRECEB 
	...
	dbSelectArea("SD2")
	dbSetOrder(3)
	dbSeek(xFilial("SD2") + cNFis + cSeri)
	...
	While !EOF("SD2") .And. AllTrim(SD2->D2_DOC + SD2->D2_SERIE) == AllTrim(cNFis + cSeri)
		...
		If !(SD2->D2_TES $ cTpes)                 
			...
			//Mensagem da TES
			/*If Len(Alltrim(SF4->F4_MENPAD)) > 0
				dbSelectArea("SZ1")
				dbSetOrder(1)
				dbSeek(xFilial("SZ1") + SF4->F4_MENPAD)
				cMenN += Alltrim(SZ1->Z1_MENSAGE)	
			EndIf*/  //Euroamerican Ajustar

			If Len(Alltrim(SF4->F4_FORMULA)) > 0
				dbSelectArea("SM4")
				dbSetOrder(1)
				dbSeek(xFilial("SM4") + SF4->F4_FORMULA)
				cMenN += Alltrim(SM4->M4_FORMULA)	
			EndIf
...
// Mensagem de retira de container
cQry := " SELECT	SUM(B6_SALDO) SALDO " + ENTER
...
cQry += " 			AND B6_FILIAL + B6_CLIFOR + B6_LOJA = '" + SF2->F2_FILIAL + SF2->F2_CLIENTE + SF2->F2_LOJA + "' "
...
cMsgCtn := ""			
dbSelectArea("QRY")
If !QRY->(EOF()) .AND. QRY->SALDO > 0 //Alterado 20/12/17
	cMsgCtn := "==========================================================" + ENTER     
	cMsgCtn += "SALDO DE CONTAINERES À RETORNAR: " + Transform(QRY->SALDO, "@E 9,999") + ENTER     
	cMsgCtn += "OS SALDOS ACIMA DE 180 DIAS SERÃO FATURADOS CONFORME RICMS" + ENTER     
	cMsgCtn += "==========================================================" + ENTER     
EndIf   
QRY->(dbCloseArea())

If !Empty(cPedF)
	aAdd(aMensagem, {"********** PEDIDO DE VENDA: " + cPedF + " **********", .F. })
EndIf                             

If !Empty(cMenN)        
	For i := 1 to Len(AllTrim(cMenN)) Step nTam
		aAdd(aMensagem, {Subs(cMenN, i, nTam)    , .F. })
	Next i
EndIf

If !Empty(cMenP)        
	For i := 1 to Len(AllTrim(cMenP)) Step nTam
		aAdd(aMensagem, {Subs(cMenP, i, nTam) , .F. })
	Next i
EndIf                                     

If !Empty(cCobC)                             
	cCobC := "End. Cobrança: " + cCobC  
	For i := 1 to Len(AllTrim(cCobC)) Step nTam     
		aAdd(aMensagem, {Subs(cCobC, i, nTam), .F. })
	Next i
EndIf

If !Empty(cPedC)
	aAdd(aMensagem, {"S/ Pedido Nº : " + cPedC, .F. })
EndIf                             
                         
/*If !Empty(cLotC)
	aAdd(aMensagem, "Lotes: " + cLotC)
EndIf

If !Empty(mv_par05)
	aAdd(aMensagem, mv_par05)
EndIf

If !Empty(mv_par06)
	aAdd(aMensagem, mv_par06)
EndIf     */

If !Empty(cMsgVCO)          
	aAdd(aMensagem, {cMsgVCO, .F. })
EndIf

If !Empty(cMsgCtn)          
	aAdd(aMensagem, {cMsgCtn, .F. })
EndIf
