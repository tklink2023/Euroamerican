...
If lProc
	MsgRun("Preparando a geração do Pedido em PDF. Aguarde.....", "Aguarde", {|| RunImp() })
else
...
if oPrn:GetViewPDF()
		If File( AllTrim( cPathDest ) + AllTrim( cRelName ) )
	       PCEmail() 			
			...
			MsgAlert("Nao foi encontrado o arquivo de pedido em PDF, E-mail não foi enviado","Atencao")
		EndIf
else
	MsgAlert("Não foi gerado o pedido de venda","Atencao")		
endif
...
Local cEmailUsr :=Alltrim(UsrRetMail(SC7->C7_USER)) // Alterado por Paulo Lenzi 17/04/2024
Local cEmailSol := ""
...
IF Empty(Alltrim(cEmailUsr)) // Alteração realizada no dia 15/10/2024
    cEmailUsr := U_SeachEmail(SC7->C7_USER)  //IIF((Empty(Alltrim(UsrRetMail(SC7->C7_USER)))),cPara,Alltrim(UsrRetMail(SC7->C7_USER)))
Endif
...
conout("*********************************************************************************")
conout("## Pedido enviado: ("+cRelName+")")
conout("## Comprado .....: "+alltrim(cEmailUsr))
conout("## INICIO........: "+dtoc(dDatabase)+" as "+time() )
conout("*********************************************************************************")
...
/*/{Protheus.doc} SeachEmail
description
@type function
@version 1.0
@author Paulo Lenzi
@since 15/10/2024
@param cCodComprador, character, param_description
@return variant, return_description
/*/
User Function SeachEmail(cCodComprador)
	...
	cQuery := "SELECT USR_NOME, USR_EMAIL FROM SYS_USR WHERE D_E_L_E_T_ = ' ' AND USR_ID = '"+cCodComprador+"' "
    ...
    (cAlias)->( dbGotop() )
    While (cAlias)->( !Eof() )
	  cUserNome := (cAlias)->USR_NOME
  	  cUserMail := (cAlias)->USR_EMAIL
      (cAlias)->(dbSkip() )           
    ENDDO
    (cAlias)->(dbCloseArea())   

   if Empty(cUserMail)
       MsgAlert("Comprador "+cUserNome+" sem Senha no cadastro de usuarios do sistema. ")
	endif
